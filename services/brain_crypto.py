"""Crypto brain — momentum + macro + vol + USD backdrop.

Composes existing terminal views (watchlist metrics, macro regime, vol read,
USD index). No FMP company fundamentals. Research synthesis only.
"""
...
def _build(inst: reg.TrackedInstrument, regime_label: str | None, *, include_context: bool) -> dict:
    if inst.asset != "crypto":
        raise ValueError(f"{inst.id!r} is not crypto")
    ...
    usd = usd_headwind_crypto(_usd_1w())
...
def verdict(instrument: str) -> dict:
    """Fused crypto conviction for one tracked symbol."""
...
def screen(symbols: list[str] | None = None, limit: int = 25) -> dict:
    """Rank conviction across crypto registry (or explicit symbols)."""
    if symbols:
        tickers = []
        for ref in symbols:
            inst = reg.resolve(ref.strip())
            if inst.asset != "crypto":
                raise ValueError(f"{ref!r} is not crypto")
            tickers.append(inst)
    else:
        tickers = [i for i in reg.list_all() if i.asset == "crypto"]
