...
def dashboard(horizon: int = Query(5, ge=1, le=21, description="Forecast horizon in days")) -> Envelope:
    ...
    return Envelope(data=vol_svc.dashboard(horizon), provider="derived (yfinance)", freshness=_FRESHNESS)
...
def one(
    ...
    return Envelope(data=data, provider="derived (yfinance)", freshness=_FRESHNESS)
